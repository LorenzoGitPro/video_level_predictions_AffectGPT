...
########################################################################################""
        if is_main_process():
            ...
            print(f"[TensorBoard] Logs dans : {self.output_dir}/tensorboard")
        else:
            self.writer = None
            print(f"[TensorBoard] Logs non activés pour le processus secondaire")
            ########################################################################
...
##############################################################
    ...
    def train_epoch(self, epoch):
        ...
        # ← AJOUTER
        if self.writer is not None and train_stats is not None:
            for k, v in train_stats.items():
                self.writer.add_scalar(f"train/{k}", float(v), epoch)
