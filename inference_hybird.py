...
def get_name2cls(dataset):
    if dataset == 'BAH':             return BAH_Dataset()
...
if __name__ == "__main__":
    ...
    for ii, ckpt_3 in enumerate(whole_ckpt3s):
        ...
        if ii == 0: # first-round: initialize models
            ...
            model = model.float()  # !!! this line here was added !!!
        ...
        for dataset in process_datasets:
            ...
            ################################## xp_bah.sh ##################################
            tag = os.environ.get("EXP_TAG", "default")
            use_transcript = os.environ.get("USE_TRANSCRIPT", "1")
            mode = "with_transcript" if use_transcript == "1" else "no_transcript"

            save_root = f"output/results-BAH/{mode}_{tag}"
            ...
            ############################## xp_bah.sh ##############################
                ...
                if use_transcript == "0":
                    subtitle_input = "[Transcript missing, use only audio/video]"
                else:
                ...
                #prompt = dataset_cls.get_prompt_for_multimodal(face_or_frame,subtitle, user_message) # commented out to perform standard (without xp_bah.sh) inference
            #########################################################################
